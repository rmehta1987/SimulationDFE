...
def main(argv):
    ...
    path = "Experiments/saved_posteriors_nfe_infer_lof_selection_monarch_nsf{}".format(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'))
    ...
    for i in range(0,rounds):
        ...
        if prop_metric < 0.6:
            np.savetxt(f'theta_prop_{i}.txt',theta.squeeze().cpu().numpy(),fmt='%.9e')
        del prop_metric
