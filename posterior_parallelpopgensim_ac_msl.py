...
def generate_moments_sim_data(prior: float) -> torch.float32:
    ...
    while rerun:
        ...
        if abs(np.max(fs)) > 10 or np.any(np.isnan(fs)):
            ...
            rerun = False
    fs = torch.poisson(torch.tensor(fs.project([sample_size]) * theta_mis)).type(torch.float32)
    return fs
...
def main(argv):
    ...
    true_x = load_true_data('emperical_missense_sfs_msl.npy', 0)
    print("True data shape (should be the same as the sample size): {} {}".format(true_x.shape[0], sample_size*2-1))



    path = "Experiments/saved_posteriors_msl{}".format(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'))
