...
class ForwardKLOptimizer(DivergenceOptimizer):
    ...
    def _loss_q_proposal(self, x_o: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        with torch.no_grad():
            ...
            #weights = self.weight_transform(logweights.exp()+1e-6)
            #weights /= weights.sum()
            #weights = torch.nn.functional.softmax(self.weight_transform(logweights),dim=1)
            #logsumexp method:
            #logweights = logp - logq
            #mean_log_weights = torch.logsumexp(logweights, 0)
            #normed_logweights = logweights - mean_log_weights
            #weights = normed_logweights.exp()
            #method from paper
            #logweights = self.weight_transform(logp - logq)
            #logweights = logweights-torch.max(logweights)
            #sumweights = torch.logsumexp(logweights,0)
            #weights = logweights-sumweights
            #weights = weights.exp()
            #truncated weights
            logweights = logp - logq
            logweights_max = logweights.max()
            original_weights = torch.exp(logweights - logweights_max)
            average_weight = torch.mean(original_weights)
            weights = torch.minimum(original_weights,
                                  average_weight * np.sqrt(self.n_particles))
            logweights2 = torch.log(weights)
            sumweights = torch.logsumexp(logweights2,0)
            weights = logweights2-sumweights
            weights = weights.exp()
            #weights /= weights.sum()
            # Paretto smoothed weights
            '''
            from sbi.samplers.importance.importance_sampling import gpdfit
            N = torch.tensor(len(weights),device=self.device)
            M = int(min(N / 5, 3 * torch.sqrt(N)))
            vals, index = weights.sort()
            largest_weigths = vals[-M:]
            mu = vals[-M]
            k, sigma = gpdfit(largest_weigths)
            if k > 1.0:
                print("The estimator has infinite variance")
            from sbi.samplers.vi.vi_quality_control import GerneralizedParetto
            p = GerneralizedParetto(mu, sigma, k)
            M2 = (torch.arange(1, M + 1) - 0.5) / M
            new_weights = p.inv_cdf(M2.to(self.device))
