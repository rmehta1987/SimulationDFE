...
def main(argv):
    ...
    embedding_net = SummaryNet(sample_size*2-1, [64, 32, 32]).to(the_device)
    ...
    '''
    num_rounds=500
    min, mean, max = restricted_simulations_with_embedding_find_min(proposal, embedding_net, sinkhorn, 
                                                             batch_size, num_rounds, embedding_true_x_norm, simulator)
    print("lowest min {}, max min {}, mean min {}".format(np.min(min), np.max(min), np.mean(min)))
    print("mean loss {}".format(np.mean(mean)))
    print("max mean loss {}".format(np.mean(max)))
    ...
    if load_classifier:
        restriction_estimator = torch.load('nfe_restriction_classifier_lof_embedding.pkl')
        ...
        for i in tqdm(range(0,estimator_round)):
            if i == 0:
                theta, x = restricted_simulations_with_embedding(proposal, embedding_net, rmin, sinkhorn, batch_size, embedding_true_x_norm, simulator, sample_size=150)
            else:
        ...
        torch.save(save_estimator, 'nfe_restriction_classifier_lof_embedding_more_strict.pkl')
        del save_estimator
    print("Finsiehd")
