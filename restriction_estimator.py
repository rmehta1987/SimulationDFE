...
class RestrictionEstimator:
    def __init__(
        ...
        self._device = device  # TODO hot fix to prevent the tests from crashing
    ...
    def append_simulations(self, theta: Tensor, x: Tensor, data_device: Optional[str] = None) -> "RestrictionEstimator":
        r"""
    ...
    def train(
        ...
        criterion = nn.CrossEntropyLoss(importance_weights.to(self._device), reduction="none")
        ...
        while epoch <= max_num_epochs and not self._converged(epoch, stop_after_epochs):
            ...
            for parameters, observations in train_loader:
                optimizer.zero_grad()
                parameters = parameters.to(self._device)
    ...
    def restrict_prior(
        ...
        return RestrictedPrior(self._prior, accept_reject_fn, device=self._device)
...
def get_classifier_thresholder(
    ...
    valid_val_theta = validation_theta[validation_label.bool()].to(device)
        ...
        if sample_with == "rejection":
            ...
            if save_acceptance_rate:
                ...
                if show_progress_bars:
                    print(f"""The `RestrictedPrior` rejected {(1.0 - acceptance_rate) * 100:.1f}% of prior samples. You will get a speed-up of {(1.0 / acceptance_rate - 1.0) * 100:.1f}%.""")
        elif sample_with == "rejection2":
            samples, acceptance_rate = accept_reject_sample(
    ...
    def log_prob(
        ...
        with torch.set_grad_enabled(track_gradients):
            ...
            masked_log_prob = torch.where(
                ...
                torch.tensor(float(-10000.0), dtype=torch.float32, device=self._device),
            )

            if prior_acceptance_params is None:
                prior_acceptance_params = dict()  # use defaults
            if norm_restricted_prior:
                log_factor = torch.log(self.prior_acceptance(**prior_acceptance_params))
            else:
                log_factor = 0
