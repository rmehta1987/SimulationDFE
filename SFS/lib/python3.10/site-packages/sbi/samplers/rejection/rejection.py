        ...
        if (
            ...
            if sample_for_correction_factor:
                ...
                if alternative_method is not None:
                    ...
                    `{alternative_method}` there have been currently {how_many_times_warned} warnings."""
                    

                logging.warning(warn_msg)

            leakage_warning_raised = True  # Ensure warning is raised just once.
        if (num_sampled_total > 1000 and acceptance_rate < warn_acceptance and leakage_warning_raised):
            how_many_times_warned+=1
            if how_many_times_warned % 20:
                warn_msg = f"There have been {how_many_times_warned} warnings"
                logging.warning(warn_msg)
            if how_many_times_warned > 100:
                warn_msg = f"There have been {how_many_times_warned} warnings. returning limited samples"
                logging.warning(warn_msg)
                samples = torch.cat(accepted)
                if samples.shape[0] > 0 and samples.shape[0] < num_samples:
                    return samples, as_tensor(acceptance_rate)
                else:
                    return None, None
