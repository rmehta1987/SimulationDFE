    ...
    **kwargs,
    ...
    while num_remaining > 0:
        ...
        if (
            ...
            leakage_warning_raised = True  # Ensure warning is raised just once.
        if how_many_times_warned > 0:
            how_many_times_warned+=1
            if how_many_times_warned > 50:
                warn_msg = f"There have been {how_many_times_warned} warnings. returning limited samples or none, {len(accepted)}"
                logging.warning(warn_msg)
                if any(accepted):
