...
flags.DEFINE_integer('num_sim', 500, 'How many simulations to run')
...
def main(argv):
    ...
    #embedding_net = SummaryNet3(sample_size*2-1).to(the_device)
    #embedding_net = ConceptNBMNarySparse(sample_size*2-1,sample_size*2-1,nary=None,num_bases=10000, dropout=0.2, bases_dropout=0.1, batchnorm=True)
    #embedding_net = SummaryNet(sample_size*2-1).to(the_device)
    embedding_net = SummaryNet5(sample_size*2-1).to(the_device)
    ...
    #get_sim_data('sfs_lof_sim_data.npy')
    ...
    path = "Experiments/saved_posteriors_nfe_infer_lof_selection_1_{}".format(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'))
    ...
    for i in range(0,rounds):
        ...
        potential_function_estimator = infer_posterior.append_simulations(theta, x, data_device=the_device ).train(force_first_round_loss=True, learning_rate=5e-3, training_batch_size=10, show_train_summary=True)
